...
class UrTube:
    ...
    def __init__(self):
        self.users = []  # список пользователей (объекты класса User)
        self.videos = []  # список видео (объекты класса Video)

    def log_in(self, login, password):
        for user in self.users:
            if login == str(user) and hash(password) == hash(user.password):
                self.current_user = user
                break
            elif login == str(user) and hash(password) != hash(user.password):
    ...
    def register(self, nickname, password, age):
        ...
        if not self.users:  # Проверяем, пустой ли список
